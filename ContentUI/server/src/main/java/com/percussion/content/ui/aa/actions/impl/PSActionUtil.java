...
public abstract class PSActionUtil
{
      ...
      String slotid)
   {
      ...
      {
         if (!processed.contains(pair.getFirst()))
         {
            PSNodeDefinition def = PSContentTypeHelper.findNodeDef(pair
               .getFirst());
            defs.add(def);
            processed.add(pair.getFirst());
         }
      }
   ...
   public static Map<String, IPSGuid> resolveSiteFolders(Object siteNameObj,
      Object folderPathObj) throws PSSiteManagerException, PSErrorException
   {
