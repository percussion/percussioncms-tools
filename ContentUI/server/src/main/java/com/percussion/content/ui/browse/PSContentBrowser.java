...
public class PSContentBrowser
{
   ...
   static public String getSites() throws JSONException
   {
      ...
      for (IPSSite site : sites)
      {
         ...
         jsonObj.put(COLUMN_DESCRIPTION, site.getDescription());
      ...
      PSAssemblyException
   {
      ...
      for (PSItemSummary summary : summaries)
      {
         if (allowedContentTypeIds == null)
         {
            ...
            List<IPSNodeDefinition> allowedNodeDefs = PSActionUtil
               .getAllowedNodeDefsForSlot(slotId);
            for (IPSNodeDefinition def : allowedNodeDefs)
               allowedContentTypeIds.add(def.getGUID().getUUID());
         }
   ...
   /**
    * Returns content type label for the given content type name.
    * @param contentTypeName the content item name to get a label for.
    * Not <code>null</code>.
    * @return never <code>null</code>. If the content type can't be found,
    * the content type name is returned instead of a label.
    */
   static private String getContentTypeLabel(final String contentTypeName)
   {
      if (StringUtils.isBlank(contentTypeName))
      {
         throw new IllegalArgumentException(
               "Content type name must be specified");
      }
      final IPSContentDesignWs designWs =
            PSContentWsLocator.getContentDesignWebservice();
      final List<IPSCatalogSummary> contentTypes =
            designWs.findContentTypes(contentTypeName);
      return contentTypes.isEmpty()
            ? contentTypeName
            : contentTypes.get(0).getLabel();
   }
         ...
         Map<String, String> iconMap) throws JSONException
   {
      ...
      jsonObj.put(COLUMN_DESCRIPTION,
            getContentTypeLabel(summary.getContentTypeName()));
   ...
   public static String getNewItemUrlBySiteFolderPath(String parentPath,
      String ctypeid) throws PSSiteManagerException, PSErrorException
   {
