...
public class PSSearchFieldOperators
{
   ...
   public static String getInputOperator(PSSearchField field)
   {
      ...
      else if (field.isNumberValue())
      {
         if (operator.equals(PSSearchField.OP_BETWEEN))
            ...
            result = PSSearchFieldEditor.OP_EQUALS;
         else if (operator.equals(PSSearchField.OP_GREATERTHAN))
            result = PSSearchFieldEditor.OP_GREATER_THAN;
         else if (operator.equals(PSSearchField.OP_LESSTHAN))
            result = PSSearchFieldEditor.OP_LESS_THAN;
      }
      else if (field.isTextValue())
      {
         result = PSSearchFieldEditor.OP_CONTAINS;
         if (operator.length() > 0 && !operator.equals(PSSearchField.OP_EQUALS))
         {
            if (value.startsWith("%") && value.endsWith("%"))
               result = PSSearchFieldEditor.OP_CONTAINS;
            else if (value.startsWith("%"))
               result = PSSearchFieldEditor.OP_ENDS_WITH;
            else if (value.endsWith("%"))
               result = PSSearchFieldEditor.OP_STARTS_WITH;
         }
         else if (operator.equals(PSSearchField.OP_EQUALS))
            result = PSSearchFieldEditor.OP_EXACT;
      }
   ...
   public static String getOutputOperator(PSSearchField field, String operator)
   {
      ...
      if (field.isDateValue())
      {
         if (operator.equals(PSSearchFieldEditor.OP_AFTER))
            result = PSSearchField.OP_GREATERTHAN;
         else if (operator.equals(PSSearchFieldEditor.OP_BEFORE))
            result = PSSearchField.OP_LESSTHAN;
         else if (operator.equals(PSSearchFieldEditor.OP_BETWEEN))
            result = PSSearchField.OP_BETWEEN;
         else if (operator.equals(PSSearchFieldEditor.OP_ON))
            result = PSSearchField.OP_EQUALS;
      }
      else if (field.isNumberValue())
      {
         if (operator.equals(PSSearchFieldEditor.OP_BETWEEN))
            result = PSSearchField.OP_BETWEEN;
         else if (operator.equals(PSSearchFieldEditor.OP_EQUALS))
            result = PSSearchField.OP_EQUALS;
         else if (operator.equals(PSSearchFieldEditor.OP_GREATER_THAN))
            result = PSSearchField.OP_GREATERTHAN;
         else if (operator.equals(PSSearchFieldEditor.OP_LESS_THAN))
            result = PSSearchField.OP_LESSTHAN;
      }
      else if (field.isTextValue())
      {
         result = PSSearchField.OP_LIKE;
         if (operator.equals(PSSearchFieldEditor.OP_EXACT))
            result = PSSearchField.OP_EQUALS;
      }
      ...
      PSSearchField field)
   {
      ...
      else if (field.isTextValue())
      {
         ...
         if (value.trim().length() > 0)
         {
            if (!operator.equals(PSSearchFieldEditor.OP_EXACT))
            {
               if (operator.equals(PSSearchFieldEditor.OP_STARTS_WITH))
               {
                  if (!value.endsWith("%"))
                     value += "%";
               }
               else if (operator.equals(PSSearchFieldEditor.OP_CONTAINS))
               {
                  if (!value.startsWith("%"))
                     value = "%" + value;
                  if (!value.endsWith("%"))
               ...
               else if (operator.equals(PSSearchFieldEditor.OP_ENDS_WITH))
               {
                  if (!value.startsWith("%"))
                     value = "%" + value;
               }
   ...
   static
   {
      ms_dateOperators.add(PSSearchFieldEditor.OP_ON);
      ms_dateOperators.add(PSSearchFieldEditor.OP_BEFORE);
      ms_dateOperators.add(PSSearchFieldEditor.OP_AFTER);
      ms_dateOperators.add(PSSearchFieldEditor.OP_BETWEEN);
   };
   ...
   static
   {
      ms_numberOperators.add(PSSearchFieldEditor.OP_EQUALS);
      ms_numberOperators.add(PSSearchFieldEditor.OP_GREATER_THAN);
      ms_numberOperators.add(PSSearchFieldEditor.OP_LESS_THAN);
      ms_numberOperators.add(PSSearchFieldEditor.OP_BETWEEN);
   };
   ...
   static 
   {
      ms_textOperators.add(PSSearchFieldEditor.OP_STARTS_WITH);
      ms_textOperators.add(PSSearchFieldEditor.OP_CONTAINS);
      ms_textOperators.add(PSSearchFieldEditor.OP_ENDS_WITH);
      ms_textOperators.add(PSSearchFieldEditor.OP_EXACT);
   };
}
