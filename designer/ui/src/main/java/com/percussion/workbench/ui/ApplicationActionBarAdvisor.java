...
import com.percussion.workbench.ui.actions.PSNewWizardMenu;
...
// Borrowed from org.eclipse.ui.internal.ide.WorkbenchActionBuilder
// to disable UI unnecessary for standalone Rhythmyx Workbench
{
...
//       menuBar.add(createProjectMenu());
   ...
   /**
    * Listens for selection changes on the main selection service and 
    * re-broadcasts them to all listeners as if it was a selection provider.
    *
    * @author paulhoward
    */
   private class SelectionMapper implements ISelectionProvider,
         ISelectionListener
   {
      //see interface
      ...
      public void setSelection(ISelection selection) 
      {
         ...
         for (final ISelectionChangedListener l : mi_listeners) 
         {
      ...
      /**
       * Contains the currently registered listeners. Never <code>null</code>.
       */
      private Collection<ISelectionChangedListener> mi_listeners = 
         new ArrayList<ISelectionChangedListener>();

      /**
       * Contains the most recent selection or the empty selection if there
       * isn't one. Never <code>null</code>.
       */
      private ISelection mi_selection = StructuredSelection.EMPTY;
   }   
...
//         };
         ...
         SelectionMapper selmap = new SelectionMapper();
         getWindow().getSelectionService().addSelectionListener(selmap);
         this.newWizardMenu = new PSNewWizardMenu(getWindow(), null, selmap);
...
//      menu.add(new GroupMarker(IWorkbenchActionConstants.MRU));
   ...
   {
      MenuManager menu = new MenuManager("&Edit", //IDEWorkbenchMessages.Workbench_edit,
...
//   }
   ...
   {
      MenuManager menu = new MenuManager("&Window", // IDEWorkbenchMessages.Workbench_window,
...
//         String openText = IDEWorkbenchMessages.Workbench_openPerspective;
         MenuManager changePerspMenuMgr = new MenuManager("Open Perspective", //openText,
      ...
      }
      {
         MenuManager showViewMenuMgr = new MenuManager(
               IDEWorkbenchMessages.Workbench_showView, "showView");
...
//            IDEWorkbenchMessages.Workbench_shortcuts, "shortcuts"); //$NON-NLS-1$
   ...
   {
      MenuManager menu = new MenuManager("&Help", // IDEWorkbenchMessages.Workbench_help,
...
//            .getPreferenceStore().getBoolean(prefId);
      boolean addExtraSeparators = false;
...
//       getActionBarConfigurer().getStatusLineManager().remove(statusLineItem);
       if (pageListener != null) {
           m_window.removePageListener(pageListener);
           pageListener = null;
       }
...
//   private IWorkbenchAction /*exitAction,*/ connectOtherAction/*, preferenceAction*/;
...
//      register(exitAction);
//          ActionFactory TEST_ACTION = new ActionFactory("TEST_ACTION")
//          {
//
...
//                IWorkbenchAction action = new PSConnectDialogAction(window);
//                action.setId(getId());
//
//                action.setText("&Test");
//                action.setToolTipText("A Reusable Test Action");
//                return action;
//             }
//
//          };
//          connectOtherAction = TEST_ACTION.create(window);
//          register(connectOtherAction);
//          
...
//        fileMenu.add(connectOtherAction);
