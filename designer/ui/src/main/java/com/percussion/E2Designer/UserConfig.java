...
/**
 * Reads all of the user configuration information from the server and stores
 * it for local access. Each UserConfig object is for a specific user set when
 * the object is created.
 * <p>
 * User config info is stored as key,value pairs. The user can query keys,
 * add keys and remove specified keys. All pairs can be removed, but this should
 ...
 * various types.
 * <p>
 * All changes in configuration are kept locally until the flush method is called
 * or the object is garbage collected. At that point in time, the configuration
 * data is written to the server. If the write back is occurring because of a
 * finalize call and the server can't be contacted, any changes in the config
 * data since the last flush will be lost. The object is smart about 
...
public class UserConfig
{
    ...
    * @throws PSAuthorizationException if the specified user and pw could not
    * be validated with designer access on the server
    */
   static public UserConfig createConfig(PSObjectStore Store)
               throws PSServerException,
                 PSAuthorizationException,
                 PSAuthenticationFailedException
   {
      ms_Store = Store;
      ms_theConfig = new UserConfig(ms_Store.getUserConfiguration());
    ...
    */
   static public UserConfig getConfig() 
         throws UninitializedException
   {
      if (null == ms_theConfig)
         throw new UninitializedException(
            E2Designer.getResources().getString("ConfigNotInit"));
      return ms_theConfig;
   }

   /**
    * Returns <code>true</code> if there are no properties in the configurator,
   ...
   /**
    * Looks for the specified key. If found, tries to convert it and returns
    * the result.
    *
    ...
    * otherwise <code>false</code> is returned. If no key is found, bDefaultValue
    * is returned.
    *
    * @see #setValue
    */
   ...
   /**
    * Sets the specified key to the supplied value, creating it if necessary.
    * If strKey is null or empty, nothing is done and no error occurs.
    *
    * @see #getValue
    */
   ...
   /**
    * Adds an entry to the config using the supplied key and value. Use getInteger
    ...
    * the requested name is not found, defaultValue is returned. The value should
    * be called against keys created with setInteger.
    *
    * @param strKey the key to look up
    *
    * @param defaultValue is returned if a key by the supplied name is not found
    *
    * @returns the integer associated with strKey, or defaultValue if the key
    ...
    *
    * @throws IllegalArgumentException if strKey is null or empty or iaValues
    * is null or empty
    */
   public void setIntArray(String strKey, int [] iaValues) 
   {
      if (null == strKey || null == iaValues || 0 == strKey.length() || iaValues.length <= 0)
      ...
      if(iaValues.length > 1)
      {
         ...
         {
            strValue = strValue+DELIMITER+iaValues[i]; //put a delimiter between each value
         }
      }
   ...
   /**
    * Returns an array of <code>int</code> associated with the supplied key. If the key cannot be
    * found, null is returned.
    *
    * @param strKey a non-null, non-empty string; keys are case sensitive
    *
    * @returns an array of <code>int</code> associated with the supplied key or null if the 
    ...
    *
    * @throws IllegalArgumentException if strKey is null or empty or strValue
    * is null
    */
   ...
   /**
    * Appends to the strValue corresponding to an existing key (if strKey is found)
  ...
  public void setValue(String strKey, String strValue, boolean append)
  {
   ...
   /**
    * Parses the string value associated with the specified key using the delimiter
   * and returns an array of strings (each string is a token)
    *
    * @param strKey - a non-null, non-empty string, keys are case sensitive
    *
  ...
  public String[] getStringTokens(String strKey)
  {
    ...
    else
    {
      ...
      if (strExistingValue != null)
      {
            UserConfigHelper uch = new UserConfigHelper();
...
//               System.out.println("............Token = "+(String)v.get(i));
   ...
   /**
   * Returns true if the specified string is found in the delimiter separated list
   * of the string values associated with the specified key
    *
    * @param strKey - a non-null, non-empty string, keys are case sensitive
    * @param strToken - a non-null, non-empty string, to be searched for in the existing
   * values
    *
    * @throws IllegalArgumentException if strKey is null or empty or strToken is null
    */
  public boolean isStringTokenPresent(String strKey, String strToken)
  {
    ...
    /**
   * Removes the first specified string strToken if it is found in the delimiter
   * separated list of the string values associated with the specified key
    *
    * @param strKey - a non-null, non-empty string, keys are case sensitive
    * @param strToken - a non-null, non-empty string, to be searched for in the existing
   * values
  ...
  public void removeStringToken(String strKey, String strToken)
  {
    ...
    * returned, try again later. There is a remote chance that the user's
    * access was changed to something less than designer, which would also
    * cause a false return. If this happens, the configuration information 
    * will be lost.
    */
    ...
    *
    * @throws IllegalArgumentException if uc is null
    */
   private UserConfig(PSUserConfiguration uc)
   {
      ...
      if (null == uc)
      {
         ...
         throw new IllegalArgumentException(
            ...
            E2Designer.getResources().getString("NullArgFormat"),
            astrParams));
      }
      m_Properties = uc;
   }
   ...
   /**
    * Delimiter used for storing multiple string tokens in a string value for
   * a key. Note:Must be a single char and must stay the same between versions.
    */
  public static final String DELIMITER = ";";

   /**
    * The escape char is the character that is inserted in a string just before 
    * the chars that are the same as the DELIMITER char. This allows differenciating
    * of the chars within a string and chars that are DELIMITERS.
    */
    ...
    */
   static private UserConfig ms_theConfig = null;

   /**
    * This is the object store that maintains the properties on the server.
    * We need to keep it around to write the configuration later.
    */
    ...
    * true</code>, the properties will be written back to the server when 
    * flush is called or the object is destroyed.
    */
