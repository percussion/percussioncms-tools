...
/**
 * the class to manage the UI for UTMultiLineTable.
 */
public class UTMultiLineBasicTableUI extends BasicTableUI {
  

  public UTMultiLineBasicTableUI() {
    super();
  }
  ...
  private Dimension createTableSize(long width) {
   ...
   if (tcr instanceof UTMultiLineCellRenderer) {
     height[j] = Math.max(height[j], ((UTMultiLineTable)table).getHeight((String)tm.getValueAt(j,i), colWidth));
   }
      }
      i++;
    }
  ...
  public void paint(Graphics g, JComponent c) {
    ...
    Rectangle rowRect = new Rectangle(0, 0,
                  tableWidth,
                  ((UTMultiLineTable)table).getRowHeight(firstIndex) +
                  rowMargin);
    rowRect.y = 0;
    for (int i=0; i<firstIndex; i++) {
      rowRect.y += ((UTMultiLineTable)table).getRowHeight(i) + rowMargin;
    }
   ...
   paintRow(g, index);
      ...
      rowRect.y += ((UTMultiLineTable)table).getRowHeight(index) + rowMargin;
    }
  ...
  private void paintHorizontalLines(Graphics g) {
    ...
    int y = - rowMargin;
    for (int i=0; i<firstIndex; i++) {
      y += ((UTMultiLineTable)table).getRowHeight(i) + rowMargin;
    }

    for (int index = firstIndex; index <= lastIndex; index ++) {
      y += ((UTMultiLineTable)table).getRowHeight(index) + rowMargin;
  ...
  private void paintRow(Graphics g, int row) {
    ...
    cellRect.height = ((UTMultiLineTable)table).getRowHeight(row) + spacing.height;
    cellRect.y = 0;
    for (int i=0; i<row; i++) {
      cellRect.y += ((UTMultiLineTable)table).getRowHeight(i) + spacing.height;
    }
