...
/**
 * Catalogs locales from the server.
 */
public class CatalogLocales
{
   /**
   ...
   /**
    * Catalogs and caches the locales from the server.
    ...
    * <code>false</code> to use cached data if locales have already been
    * cataloged.
    * 
    * @return The locale language string identifiers, never <code>null</code> or
    * empty unless an error occurs.
    */
   public static List<String> getCatalog(PSDesignerConnection conn, 
      boolean forceCatalog)
   {
      if (ms_localeCache == null || forceCatalog)
      {
         ms_localeCache = new ArrayList<String>();
         ...
         try
         {
            ...
            catalogProps.put("RequestType", "Locale");

            Document doc = cataloger.catalog(catalogProps);
            PSCatalogResultsWalker walker = new PSCatalogResultsWalker(doc);
            while (walker.nextResultObject("locale"))
            {
               ms_localeCache.add(walker.getResultData("languageString"));
            }
         }
         catch (IOException e)
         {
      ...
      return new ArrayList<String>(ms_localeCache);
   }
   
   /**
    * List of locale identifiers, <code>null</code> until first call to 
    * {@link #getCatalog(PSDesignerConnection, boolean)}, 
    * never <code>null</code> after that.
    */
   private static List<String> ms_localeCache = null;   
}
