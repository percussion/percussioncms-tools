...
import com.percussion.desktop.deployer.objectstore.PSDeploymentServerConnectionInfo;
...
public class PSDeploymentServerConnection
{
   ...
   public static String encryptPwd(String uid, String pwd)
   {
      ...
      try {
         ...
         ms_log.error("Unable to encrypt string, returning plain text: Error: {}", PSExceptionUtils.getMessageForLog(e));
         return pwd;
      }
   ...
   public static String decryptPwd(String uid, String pwd)
   {
      ...
      try {
         ...
         ms_log.warn("Decryption failed, falling back to legacy decryption. {}",
                 PSExceptionUtils.getMessageForLog(e));
         try {
            return PSCryptographer.decrypt(PSLegacyEncrypter.getInstance(null).INVALID_CRED(), key, pwd);
         }catch (Exception ex){
            ms_log.warn("Fallback legacy decryption failed: {} Assuming clear text." , PSExceptionUtils.getMessageForLog(ex));
            return pwd;
         }
      }
         ...
         PSServerException, PSDeployException
   {
      ...
      root.setAttribute("userId", m_uid);
      root.setAttribute("password", getPassword(false));
